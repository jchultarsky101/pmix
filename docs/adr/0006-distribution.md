...
to install the tool on Windows and macOS without a Rust toolchain.
...
- **Binaries and installers come from GitHub releases**, built by
  ...
  `v*` tag: archives for Apple Silicon and Intel macOS and for x86-64 and
  ARM64 Windows, a shell installer script for macOS, a PowerShell
  installer script for Windows, and the `pmix-update` updater binary that
  upgrades an installation in place from the latest release.
...
- Linux binaries are not built yet; adding a target is a one-line change
